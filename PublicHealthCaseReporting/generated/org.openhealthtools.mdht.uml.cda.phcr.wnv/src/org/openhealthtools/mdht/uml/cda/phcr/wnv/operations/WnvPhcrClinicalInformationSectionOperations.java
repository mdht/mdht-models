...
public class WnvPhcrClinicalInformationSectionOperations extends PhcrClinicalInformationSectionOperations {
	 ...
	 * @ordered
	 */
	protected static final String GET_WNV_CASE_OBSERVATION__EOCL_EXP = "self.getObservations()->select(observation : cda::Observation | not observation.oclIsUndefined() and observation.oclIsKindOf(wnv::WnvCaseObservation))->asSequence()->first().oclAsType(wnv::WnvCaseObservation)";
	...
	public static  WnvCaseObservation getWnvCaseObservation(WnvPhcrClinicalInformationSection wnvPhcrClinicalInformationSection) {
		if (GET_WNV_CASE_OBSERVATION__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(WnvPackage.Literals.WNV_PHCR_CLINICAL_INFORMATION_SECTION, WnvPackage.Literals.WNV_PHCR_CLINICAL_INFORMATION_SECTION.getEAllOperations().get(69));
			try {
