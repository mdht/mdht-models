...
public class ToxicShockSyndromeCaseReportOperations extends PublicHealthCaseReportOperations {
	 ...
	 * @ordered
	 */
	protected static final String GET_TSS_PHCR_CLINICAL_INFORMATION_SECTION__EOCL_EXP = "self.getAllSections()->select(section : cda::Section | not section.oclIsUndefined() and section.oclIsKindOf(tss::TssPhcrClinicalInformationSection))->asSequence()->first().oclAsType(tss::TssPhcrClinicalInformationSection)";
	...
	public static  TssPhcrClinicalInformationSection getTssPhcrClinicalInformationSection(ToxicShockSyndromeCaseReport toxicShockSyndromeCaseReport) {
		if (GET_TSS_PHCR_CLINICAL_INFORMATION_SECTION__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(tssPackage.Literals.TOXIC_SHOCK_SYNDROME_CASE_REPORT, tssPackage.Literals.TOXIC_SHOCK_SYNDROME_CASE_REPORT.getEAllOperations().get(62));
			try {
	 ...
	 * @ordered
	 */
	protected static final String GET_TSS_PHCR_RELEVANT_DX_TESTS_SECTION__EOCL_EXP = "self.getAllSections()->select(section : cda::Section | not section.oclIsUndefined() and section.oclIsKindOf(tss::TssPhcrRelevantDxTestsSection))->asSequence()->first().oclAsType(tss::TssPhcrRelevantDxTestsSection)";
