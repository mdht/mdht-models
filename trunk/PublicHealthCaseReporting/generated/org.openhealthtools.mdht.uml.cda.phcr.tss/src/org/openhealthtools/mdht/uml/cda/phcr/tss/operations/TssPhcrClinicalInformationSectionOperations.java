...
public class TssPhcrClinicalInformationSectionOperations extends PhcrClinicalInformationSectionOperations {
	 ...
	 * @ordered
	 */
	protected static final String GET_TSS_CASE_OBSERVATION__EOCL_EXP = "self.getObservations()->select(observation : cda::Observation | not observation.oclIsUndefined() and observation.oclIsKindOf(tss::TssCaseObservation))->asSequence()->first().oclAsType(tss::TssCaseObservation)";
	...
	public static  TssCaseObservation getTssCaseObservation(TssPhcrClinicalInformationSection tssPhcrClinicalInformationSection) {
		if (GET_TSS_CASE_OBSERVATION__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(tssPackage.Literals.TSS_PHCR_CLINICAL_INFORMATION_SECTION, tssPackage.Literals.TSS_PHCR_CLINICAL_INFORMATION_SECTION.getEAllOperations().get(69));
			try {
