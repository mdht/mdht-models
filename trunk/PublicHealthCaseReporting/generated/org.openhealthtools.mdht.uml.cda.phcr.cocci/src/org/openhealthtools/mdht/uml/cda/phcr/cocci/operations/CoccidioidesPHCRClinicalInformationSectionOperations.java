...
public class CoccidioidesPHCRClinicalInformationSectionOperations extends PhcrClinicalInformationSectionOperations {
	 ...
	 * @ordered
	 */
	protected static final String GET_COCCIDIOIDES_CASE_OBSERVATION__EOCL_EXP = "self.getObservations()->select(observation : cda::Observation | not observation.oclIsUndefined() and observation.oclIsKindOf(cocci::CoccidioidesCaseObservation))->asSequence()->first().oclAsType(cocci::CoccidioidesCaseObservation)";
	...
	public static  CoccidioidesCaseObservation getCoccidioidesCaseObservation(CoccidioidesPHCRClinicalInformationSection coccidioidesPHCRClinicalInformationSection) {
		if (GET_COCCIDIOIDES_CASE_OBSERVATION__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(CocciPackage.Literals.COCCIDIOIDES_PHCR_CLINICAL_INFORMATION_SECTION, CocciPackage.Literals.COCCIDIOIDES_PHCR_CLINICAL_INFORMATION_SECTION.getEAllOperations().get(70));
			try {
