...
public class CoccidioidesPHCRTreatmentInformationSectionOperations extends PhcrTreatmentInformationSectionOperations {
	 ...
	 * @ordered
	 */
	protected static final String GET_COCCIDIOIDES_THERAPEUTIC_REGIMEN_ACT__EOCL_EXP = "self.getActs()->select(act : cda::Act | not act.oclIsUndefined() and act.oclIsKindOf(cocci::CoccidioidesTherapeuticRegimenAct))->asSequence()->first().oclAsType(cocci::CoccidioidesTherapeuticRegimenAct)";
	...
	public static  CoccidioidesTherapeuticRegimenAct getCoccidioidesTherapeuticRegimenAct(CoccidioidesPHCRTreatmentInformationSection coccidioidesPHCRTreatmentInformationSection) {
		if (GET_COCCIDIOIDES_THERAPEUTIC_REGIMEN_ACT__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(CocciPackage.Literals.COCCIDIOIDES_PHCR_TREATMENT_INFORMATION_SECTION, CocciPackage.Literals.COCCIDIOIDES_PHCR_TREATMENT_INFORMATION_SECTION.getEAllOperations().get(62));
			try {
