...
public class EMSSocialHistorySectionOperations extends SectionOperations {
	...
	public static boolean validateEMSSocialHistorySectionEntry1(EMSSocialHistorySection emsSocialHistorySection,
		...
		if (!EOCL_ENV.createQuery(VALIDATE_EMS_SOCIAL_HISTORY_SECTION_ENTRY1__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			emsSocialHistorySection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, EmspcrValidator.DIAGNOSTIC_SOURCE,
	...
	public static boolean validateEMSSocialHistorySectionEntry2(EMSSocialHistorySection emsSocialHistorySection,
			DiagnosticChain diagnostics, Map<Object, Object> context) {
		...
		if (!EOCL_ENV.createQuery(VALIDATE_EMS_SOCIAL_HISTORY_SECTION_ENTRY2__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			emsSocialHistorySection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, EmspcrValidator.DIAGNOSTIC_SOURCE,
