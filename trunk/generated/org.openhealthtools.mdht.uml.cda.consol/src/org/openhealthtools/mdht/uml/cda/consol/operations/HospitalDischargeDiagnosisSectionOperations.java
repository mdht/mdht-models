...
public class HospitalDischargeDiagnosisSectionOperations extends SectionOperations {
	...
	public static boolean validateHospitalDischargeDiagnosisSectionCondition(
		...
		if (!EOCL_ENV.createQuery(
			VALIDATE_HOSPITAL_DISCHARGE_DIAGNOSIS_SECTION_CONDITION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			hospitalDischargeDiagnosisSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR,
	...
	public static ProblemConcernAct getCondition(HospitalDischargeDiagnosisSection hospitalDischargeDiagnosisSection) {
		if (GET_CONDITION__EOCL_QRY == null) {
			...
			helper.setOperationContext(
				...
				ConsolPackage.Literals.HOSPITAL_DISCHARGE_DIAGNOSIS_SECTION.getEAllOperations().get(58));
			try {
