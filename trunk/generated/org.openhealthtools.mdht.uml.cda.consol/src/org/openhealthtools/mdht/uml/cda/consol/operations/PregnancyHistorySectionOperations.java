...
public class PregnancyHistorySectionOperations extends SectionOperations {
	...
	public static boolean validatePregnancyHistorySectionTemplateId(PregnancyHistorySection pregnancyHistorySection,
		...
		if (VALIDATE_PREGNANCY_HISTORY_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV == null) {
			...
			helper.setContext(ConsolPackage.Literals.PREGNANCY_HISTORY_SECTION);
			try {
		...
		if (!EOCL_ENV.createQuery(VALIDATE_PREGNANCY_HISTORY_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			pregnancyHistorySection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
					ConsolValidator.PREGNANCY_HISTORY_SECTION__PREGNANCY_HISTORY_SECTION_TEMPLATE_ID,
					ConsolPlugin.INSTANCE.getString("PregnancyHistorySectionTemplateId"),
	...
	public static boolean validatePregnancyHistorySectionCode(PregnancyHistorySection pregnancyHistorySection,
		...
		if (!EOCL_ENV.createQuery(VALIDATE_PREGNANCY_HISTORY_SECTION_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			pregnancyHistorySection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
					ConsolValidator.PREGNANCY_HISTORY_SECTION__PREGNANCY_HISTORY_SECTION_CODE,
					ConsolPlugin.INSTANCE.getString("PregnancyHistorySectionCode"),
					new Object[] { pregnancyHistorySection }));
			}
			return false;
		}
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_PREGNANCY_HISTORY_SECTION_PREGNANCY_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.entry->exists(entry : cda::Entry | not entry.observation.oclIsUndefined() and entry.observation.oclIsKindOf(consol::PregnancyObservation))";
	...
	public static boolean validatePregnancyHistorySectionPregnancyObservation(
		...
		if (!EOCL_ENV.createQuery(
			VALIDATE_PREGNANCY_HISTORY_SECTION_PREGNANCY_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			pregnancyHistorySection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
					ConsolValidator.PREGNANCY_HISTORY_SECTION__PREGNANCY_HISTORY_SECTION_PREGNANCY_OBSERVATION,
					ConsolPlugin.INSTANCE.getString("PregnancyHistorySectionPregnancyObservation"),
					new Object[] { pregnancyHistorySection }));
			}
			return false;
		}
	...
	public static EList<PregnancyObservation> getPregnancyObservations(PregnancyHistorySection pregnancyHistorySection) {
		if (GET_PREGNANCY_OBSERVATIONS__EOCL_QRY == null) {
			...
			helper.setOperationContext(
				ConsolPackage.Literals.PREGNANCY_HISTORY_SECTION,
				ConsolPackage.Literals.PREGNANCY_HISTORY_SECTION.getEAllOperations().get(58));
			try {
