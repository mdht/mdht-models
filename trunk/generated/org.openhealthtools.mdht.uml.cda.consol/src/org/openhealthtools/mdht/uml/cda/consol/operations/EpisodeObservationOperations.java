...
public class EpisodeObservationOperations extends ClinicalStatementOperations {
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_EPISODE_OBSERVATION_ONE_ENTRY_RELATIONSHIP_SUBJ__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.getEntryRelationshipTargets(vocab::x_ActRelationshipEntryRelationship::SUBJ, consol::ProblemConcernAct)->size() = 1"
		...
		if (!EOCL_ENV.createQuery(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					ConsolValidator.EPISODE_OBSERVATION__EPISODE_OBSERVATION_ONE_ENTRY_RELATIONSHIP_SUBJ,
					ConsolPlugin.INSTANCE.getString("EpisodeObservationOneEntryRelationshipSUBJ"),
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_EPISODE_OBSERVATION_EXISTS_ENTRY_RELATIONSHIP_SAS__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.getEntryRelationshipTargets(vocab::x_ActRelationshipEntryRelationship::SAS, consol::ProblemConcernAct)->size() >0"
	 ...
	 * <!-- end-user-doc -->
	 * <!-- begin-model-doc -->
	 * self.getEntryRelationshipTargets(vocab::x_ActRelationshipEntryRelationship::SAS, consol::ProblemConcernAct)->size() >0
	 *   or self.getEntryRelationshipTargets(vocab::x_ActRelationshipEntryRelationship::SAS, consol::SocialHistoryObservation)->size() > 0
	 * @param episodeObservation The receiving '<em><b>Episode Observation</b></em>' model object.
	 * @param diagnostics The chain of diagnostics to which problems are to be appended.
	 * @param context The cache of context-specific information.
		...
		if (!EOCL_ENV.createQuery(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					ConsolValidator.EPISODE_OBSERVATION__EPISODE_OBSERVATION_EXISTS_ENTRY_RELATIONSHIP_SAS,
					ConsolPlugin.INSTANCE.getString("EpisodeObservationExistsEntryRelationshipSAS"),
					new Object[] { episodeObservation }));
			}
			return false;
		}
	...
	public static boolean validateEpisodeObservationValue(EpisodeObservation episodeObservation,
		...
		if (!EOCL_ENV.createQuery(VALIDATE_EPISODE_OBSERVATION_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.WARNING, ConsolValidator.DIAGNOSTIC_SOURCE,
					ConsolValidator.EPISODE_OBSERVATION__EPISODE_OBSERVATION_VALUE,
					ConsolPlugin.INSTANCE.getString("EpisodeObservationValue"), new Object[] { episodeObservation }));
			}
			return false;
		}
		return true;
