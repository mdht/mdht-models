...
"not value.code.oclIsUndefined()))";
	...
	public static boolean validateLanguageSpokenLanguageCode(LanguageSpoken languageSpoken,
		...
		if (!EOCL_ENV.createQuery(VALIDATE_LANGUAGE_SPOKEN_LANGUAGE_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(languageSpoken)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 ...
						 ConsolPlugin.INSTANCE.getString("LanguageSpokenLanguageCode"),
						 new Object [] { languageSpoken }));
			}
			return false;
		}
				...
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 ...
						 ConsolPlugin.INSTANCE.getString("LanguageSpokenModeCode"),
						 new Object [] { languageSpoken }));
			}
			return false;
		}
