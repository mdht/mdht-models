...
public class ComponentOf_EncompassingEncounter6Operations extends EncompassingEncounterOperations {
	...
	public static boolean validateEncompassingEncounter6EffectiveTime(
		...
		if (!EOCL_ENV.createQuery(VALIDATE_ENCOMPASSING_ENCOUNTER6_EFFECTIVE_TIME__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(componentOf_EncompassingEncounter6)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
	...
	public static boolean validateEncompassingEncounter6Id(
		...
		if (!EOCL_ENV.createQuery(VALIDATE_ENCOMPASSING_ENCOUNTER6_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(componentOf_EncompassingEncounter6)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
	...
	public static boolean validateEncompassingEncounter6Location(
		...
		if (!EOCL_ENV.createQuery(VALIDATE_ENCOMPASSING_ENCOUNTER6_LOCATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(componentOf_EncompassingEncounter6)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
