...
public class EncompassingEncounter3_ResponsiblePartyOperations extends ResponsiblePartyOperations {
	...
	public static boolean validateResponsiblePartyAssignedEntityHasPersonOrOrganization(
		...
		if (!EOCL_ENV.createQuery(VALIDATE_RESPONSIBLE_PARTY_ASSIGNED_ENTITY_HAS_PERSON_OR_ORGANIZATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(encompassingEncounter3_ResponsibleParty)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
	...
	public static boolean validateResponsiblePartyAssignedEntity(
		...
		if (!EOCL_ENV.createQuery(VALIDATE_RESPONSIBLE_PARTY_ASSIGNED_ENTITY__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(encompassingEncounter3_ResponsibleParty)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
