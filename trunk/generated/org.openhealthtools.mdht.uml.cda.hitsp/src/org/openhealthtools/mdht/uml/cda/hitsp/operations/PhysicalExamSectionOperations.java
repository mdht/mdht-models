...
public class PhysicalExamSectionOperations extends PhysicalExaminationSectionOperations {
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_HITSP_PHYSICAL_EXAM_SECTION_CONDITION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(physicalExamSection))
    {
      if (diagnostics != null)
      {
        diagnostics.add
   ...
   */
    ...
    {
      OCL.Helper helper = EOCL_ENV.createOCLHelper();
      helper.setOperationContext(HITSPPackage.Literals.PHYSICAL_EXAM_SECTION, HITSPPackage.Literals.PHYSICAL_EXAM_SECTION.getEAllOperations().get(112));
      try
      {
