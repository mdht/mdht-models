...
public class AlertsSectionOperations extends SectionOperations {
	 ...
	 * @generated
	 */
	public static  boolean validateAlertsSectionTitleValue(AlertsSection alertsSection, DiagnosticChain diagnostics, Map<Object, Object> context) {
		...
		if (!EOCL_ENV.createQuery(VALIDATE_ALERTS_SECTION_TITLE_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(alertsSection)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.WARNING,
						 ...
						 CCDPlugin.INSTANCE.getString("AlertsSectionTitleValue"),
...
"value.code = '48765-2' and value.codeSystem = '2.16.840.1.113883.6.1'))";
	...
	public static EList<ProblemAct> getProblemActs(AlertsSection alertsSection) {
		if (GET_PROBLEM_ACTS__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(CCDPackage.Literals.ALERTS_SECTION, CCDPackage.Literals.ALERTS_SECTION.getEAllOperations().get(61));
			try {
