...
public class PurposeSectionOperations extends SectionOperations {
	 ...
	 * @generated
	 */
	public static  boolean validatePurposeSectionTitleValue(PurposeSection purposeSection, DiagnosticChain diagnostics, Map<Object, Object> context) {
		...
		if (!EOCL_ENV.createQuery(VALIDATE_PURPOSE_SECTION_TITLE_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(purposeSection)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.WARNING,
						 ...
						 CCDPlugin.INSTANCE.getString("PurposeSectionTitleValue"),
...
"value.code = '48764-5' and value.codeSystem = '2.16.840.1.113883.6.1'))";
	...
	public static EList<PurposeActivity> getPurposeActivities(PurposeSection purposeSection) {
		if (GET_PURPOSE_ACTIVITIES__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(CCDPackage.Literals.PURPOSE_SECTION, CCDPackage.Literals.PURPOSE_SECTION.getEAllOperations().get(61));
			try {
