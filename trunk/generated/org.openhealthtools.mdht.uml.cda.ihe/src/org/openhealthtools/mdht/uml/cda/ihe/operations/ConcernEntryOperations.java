...
"   and ((self.statusCode.code <> 'completed' and self.statusCode.code <> 'aborted') implies self.effectiveTime.high.oclIsUndefined())";
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_CONCERN_ENTRY_HAS_RELATED_OBSERVATIONS__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(concernEntry))
    {
      if (diagnostics != null)
      {
        diagnostics.add
          (new BasicDiagnostic
            (Diagnostic.ERROR,
             ...
             IHEPlugin.INSTANCE.getString("ConcernEntryHasRelatedObservations"),
             new Object [] { concernEntry }));
      }
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_CONCERN_ENTRY_RELATED_OBSERVATIONS_TYPE_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(concernEntry))
    {
      if (diagnostics != null)
      {
        diagnostics.add
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_CONCERN_ENTRY_HAS_RELATED_REFERENCES_TYPE_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(concernEntry))
    {
      if (diagnostics != null)
      {
        diagnostics.add
