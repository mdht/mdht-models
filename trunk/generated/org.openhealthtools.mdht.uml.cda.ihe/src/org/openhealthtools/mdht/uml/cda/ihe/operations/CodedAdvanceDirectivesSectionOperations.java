...
public class CodedAdvanceDirectivesSectionOperations extends AdvanceDirectivesSectionOperations {
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_CODED_ADVANCE_DIRECTIVES_SECTION_ADVANCE_DIRECTIVES_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(codedAdvanceDirectivesSection))
    {
      if (diagnostics != null)
      {
        diagnostics.add
   ...
   */
    ...
    {
      OCL.Helper helper = EOCL_ENV.createOCLHelper();
      helper.setOperationContext(IHEPackage.Literals.CODED_ADVANCE_DIRECTIVES_SECTION, IHEPackage.Literals.CODED_ADVANCE_DIRECTIVES_SECTION.getEAllOperations().get(64));
      try
      {
