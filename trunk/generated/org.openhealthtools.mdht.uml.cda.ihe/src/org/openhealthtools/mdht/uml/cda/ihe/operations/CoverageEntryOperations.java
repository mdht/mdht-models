...
public class CoverageEntryOperations extends CoverageActivityOperations {
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_COVERAGE_ENTRY_PAYER_ENTRY__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(coverageEntry))
    {
      if (diagnostics != null)
      {
        diagnostics.add
          (new BasicDiagnostic
            (Diagnostic.ERROR,
             ...
             IHEPlugin.INSTANCE.getString("CoverageEntryPayerEntry"),
             new Object [] { coverageEntry }));
      }
   ...
   */
    ...
    {
      OCL.Helper helper = EOCL_ENV.createOCLHelper();
      helper.setOperationContext(IHEPackage.Literals.COVERAGE_ENTRY, IHEPackage.Literals.COVERAGE_ENTRY.getEAllOperations().get(63));
      try
      {
