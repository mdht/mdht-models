...
public class DischargeSummaryOperations extends MedicalSummaryOperations {
	...
	* <!-- end-user-doc -->
		...
		if (!EOCL_ENV.createQuery(VALIDATE_DISCHARGE_SUMMARY_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					IHEPlugin.INSTANCE.getString("DischargeSummaryCode"), new Object[] { dischargeSummary }));
			}
	...
	public static boolean validateDischargeSummaryActiveProblemsSection(DischargeSummary dischargeSummary,
		...
		if (!EOCL_ENV.createQuery(VALIDATE_DISCHARGE_SUMMARY_ACTIVE_PROBLEMS_SECTION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			dischargeSummary)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, IHEValidator.DIAGNOSTIC_SOURCE,
	...
	public static ActiveProblemsSection getActiveProblemsSection(DischargeSummary dischargeSummary) {
		if (GET_ACTIVE_PROBLEMS_SECTION__EOCL_QRY == null) {
			...
			helper.setOperationContext(
				...
				IHEPackage.Literals.DISCHARGE_SUMMARY.getEAllOperations().get(77));
			try {
