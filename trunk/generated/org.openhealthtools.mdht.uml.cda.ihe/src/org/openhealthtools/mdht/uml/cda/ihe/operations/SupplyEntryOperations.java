...
public class SupplyEntryOperations extends SupplyActivityOperations {
	...
	*/
	...
	public static boolean validateSupplyActivityQuantity(SupplyEntry supplyEntry, DiagnosticChain diagnostics,
		...
		if (!EOCL_ENV.createQuery(VALIDATE_SUPPLY_ACTIVITY_QUANTITY__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(supplyEntry)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.WARNING, IHEValidator.DIAGNOSTIC_SOURCE,
					IHEValidator.SUPPLY_ENTRY__SUPPLY_ACTIVITY_QUANTITY,
					IHEPlugin.INSTANCE.getString("SupplyActivityQuantity"), new Object[] { supplyEntry }));
			}
		...
		if (!EOCL_ENV.createQuery(VALIDATE_SUPPLY_ACTIVITY_REPEAT_NUMBER__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					IHEPlugin.INSTANCE.getString("SupplyActivityRepeatNumber"), new Object[] { supplyEntry }));
			}
