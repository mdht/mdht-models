...
public class AllergyIntoleranceOperations extends ProblemEntryOperations {
	...
	*/
		...
		if (!EOCL_ENV.createQuery(VALIDATE_PROBLEM_OBSERVATION_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					IHEPlugin.INSTANCE.getString("ProblemObservationCodeP"), new Object[] { allergyIntolerance }));
			}
		...
		if (!EOCL_ENV.createQuery(VALIDATE_PROBLEM_OBSERVATION_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					IHEPlugin.INSTANCE.getString("ProblemObservationCode"), new Object[] { allergyIntolerance }));
			}
	...
	public static boolean validateProblemEntryValue(AllergyIntolerance allergyIntolerance, DiagnosticChain diagnostics,
		...
		if (!EOCL_ENV.createQuery(VALIDATE_PROBLEM_ENTRY_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			allergyIntolerance)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, IHEValidator.DIAGNOSTIC_SOURCE,
					IHEValidator.ALLERGY_INTOLERANCE__PROBLEM_ENTRY_VALUE,
					IHEPlugin.INSTANCE.getString("ProblemEntryValue"), new Object[] { allergyIntolerance }));
			}
