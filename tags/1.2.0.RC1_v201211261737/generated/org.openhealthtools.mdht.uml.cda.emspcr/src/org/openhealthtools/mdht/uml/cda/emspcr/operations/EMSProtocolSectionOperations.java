...
public class EMSProtocolSectionOperations extends SectionOperations {
	...
	public static boolean validateEMSProtocolSectionEntry2(EMSProtocolSection emsProtocolSection,
			DiagnosticChain diagnostics, Map<Object, Object> context) {
		...
		if (!EOCL_ENV.createQuery(VALIDATE_EMS_PROTOCOL_SECTION_ENTRY2__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			emsProtocolSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, EmspcrValidator.DIAGNOSTIC_SOURCE,
					EmspcrValidator.EMS_PROTOCOL_SECTION__EMS_PROTOCOL_SECTION_ENTRY2,
					EmspcrPlugin.INSTANCE.getString("EMSProtocolSectionEntry2"), new Object[] { emsProtocolSection }));
			}
