...
namespace facade.consol.continuityofcaredocument.author
{
    public class AssignedAuthorFacade : facade.FacadeBase
    {
		...
		*/
		...
		public List<facade.cda.AuthoringDeviceFacade> assignedAuthoringDevice()
		{
			return Set(self.assignedAuthoringDevice).FindAll( x => facade.cda.AuthoringDeviceFacade.isKindOf(x)).ConvertAll( x => new facade.cda.AuthoringDeviceFacade(x));
		}

		public facade.cda.AuthoringDeviceFacade GetOrCreateAssignedAuthoringDevice()
		{
			List<facade.cda.AuthoringDeviceFacade> lastOrDefault = assignedAuthoringDevice();
			if (lastOrDefault.Count != 0)
			{
