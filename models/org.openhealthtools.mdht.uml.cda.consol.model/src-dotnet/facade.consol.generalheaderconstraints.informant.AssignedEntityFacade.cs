...
namespace facade.consol.generalheaderconstraints.informant
{
    public class AssignedEntityFacade : facade.FacadeBase
    {
		...
		*/
		...
		public List<facade.consol.generalheaderconstraints.informant.assignedentity.PersonFacade> assignedPerson()
		{
			return Set(self.assignedPerson).FindAll( x => facade.consol.generalheaderconstraints.informant.assignedentity.PersonFacade.isKindOf(x)).ConvertAll( x => new facade.consol.generalheaderconstraints.informant.assignedentity.PersonFacade(x));
		}

		public facade.consol.generalheaderconstraints.informant.assignedentity.PersonFacade GetOrCreateAssignedPerson()
		{
			List<facade.consol.generalheaderconstraints.informant.assignedentity.PersonFacade> lastOrDefault = assignedPerson();
			if (lastOrDefault.Count != 0)
			{
