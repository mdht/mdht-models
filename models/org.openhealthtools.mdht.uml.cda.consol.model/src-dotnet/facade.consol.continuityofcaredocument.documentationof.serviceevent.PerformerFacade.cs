...
{
    public class PerformerFacade : facade.FacadeBase
    {
		...
		public PerformerFacade(POCD_MT000040Performer1 self)
		{
			this.self = self;
		}
		...
		{
			GetOrCreateTypeCode();
			self.@typeCode = SetOrAdd(self.@typeCode, x_ServiceEventPerformer.PRF);
		}
		...
		*/
		...
		public List<facade.consol.continuityofcaredocument.documentationof.serviceevent.performer.AssignedEntityFacade> assignedEntity()
		{
			return Set(self.assignedEntity).FindAll( x => facade.consol.continuityofcaredocument.documentationof.serviceevent.performer.AssignedEntityFacade.isKindOf(x)).ConvertAll( x => new facade.consol.continuityofcaredocument.documentationof.serviceevent.performer.AssignedEntityFacade(x));
		}
		...
		{
			List<facade.consol.continuityofcaredocument.documentationof.serviceevent.performer.AssignedEntityFacade> lastOrDefault = assignedEntity();
			if (lastOrDefault.Count != 0)
			{
