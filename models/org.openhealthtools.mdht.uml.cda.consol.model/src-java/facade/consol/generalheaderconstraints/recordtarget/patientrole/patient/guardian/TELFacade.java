package facade.consol.generalheaderconstraints.recordtarget.patientrole.patient.guardian;
...
public class TELFacade extends facade.FacadeBase {
	...
	*/
	...
	{
			ValidateGeneralHeaderConstraintsRecordTargetPatientRolePatientGuardianTELUse(vb, del);
	 ...
	 * Context: /GeneralHeaderConstraints/recordTarget/patientRole/patient/guardian/telecom
	 * Context Class: consol::GeneralHeaderConstraints::RecordTarget::PatientRole::Patient::Guardian::TEL
	 * Constraint Name: consol::GeneralHeaderConstraints::RecordTarget::PatientRole::Patient::Guardian::TEL::GeneralHeaderConstraintsRecordTargetPatientRolePatientGuardianTELUse
	 */
	public boolean ValidateGeneralHeaderConstraintsRecordTargetPatientRolePatientGuardianTELUse(DiagnosticChain vb, DataElementLevel del)
	{
		if (del != null && del != DataElementLevel.DEL_CDA_HEADER)
		{
			return true;
		}
		boolean result = !(Set(self).anyMatch(i1497 -> i1497.isNullFlavorUndefined())) || Set(self.getUses()).count()==0 || One(Set(self.getUses()).count())<=1;
		if (!result && vb != null)
		{
			vb.add(new BasicDiagnostic(Diagnostic.ERROR, null, 0, "Error: USRealmHeader - 2.5.12.i.d.9.iii.a use\n\t\tConformance: SHOULD contain zero or one [0..1] @use (CONF:7993)\n\t\tAnalysis: n/a\n\t\tValidation message: n/a", new Object [] { self }));
		}
		return result;
	}
