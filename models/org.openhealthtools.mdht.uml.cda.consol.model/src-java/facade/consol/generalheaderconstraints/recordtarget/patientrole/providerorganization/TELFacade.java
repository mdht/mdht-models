package facade.consol.generalheaderconstraints.recordtarget.patientrole.providerorganization;
...
public class TELFacade extends facade.FacadeBase {
	...
	*/
	...
	{
			ValidateGeneralHeaderConstraintsRecordTargetPatientRoleProviderOrganizationTELUse(vb, del);
	 ...
	 * Context: /GeneralHeaderConstraints/recordTarget/patientRole/providerOrganization/telecom
	 * Context Class: consol::GeneralHeaderConstraints::RecordTarget::PatientRole::ProviderOrganization::TEL
	 * Constraint Name: consol::GeneralHeaderConstraints::RecordTarget::PatientRole::ProviderOrganization::TEL::GeneralHeaderConstraintsRecordTargetPatientRoleProviderOrganizationTELUse
	 */
	public boolean ValidateGeneralHeaderConstraintsRecordTargetPatientRoleProviderOrganizationTELUse(DiagnosticChain vb, DataElementLevel del)
	{
		if (del != null && del != DataElementLevel.DEL_CDA_HEADER)
		{
			return true;
		}
		boolean result = !(Set(self).anyMatch(i1809 -> i1809.isNullFlavorUndefined())) || Set(self.getUses()).count()==0 || One(Set(self.getUses()).count())<=1;
		if (!result && vb != null)
		{
			vb.add(new BasicDiagnostic(Diagnostic.ERROR, null, 0, "Error: USRealmHeader - 2.5.12.i.e.4.i use\n\t\tConformance: SHOULD contain zero or one [0..1] @use (CONF:7994)\n\t\tAnalysis: n/a\n\t\tValidation message: n/a", new Object [] { self }));
		}
		return result;
	}
