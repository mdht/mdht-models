package facade.consol.generalheaderconstraints.dataenterer.assignedentity;
...
public class TELFacade extends facade.FacadeBase {
	...
	*/
	...
	{
			ValidateGeneralHeaderConstraintsDataEntererAssignedEntityTELUse(vb, del);
	 ...
	 * Context: /GeneralHeaderConstraints/dataEnterer/assignedEntity/telecom
	 * Context Class: consol::GeneralHeaderConstraints::DataEnterer::AssignedEntity::TEL
	 * Constraint Name: consol::GeneralHeaderConstraints::DataEnterer::AssignedEntity::TEL::GeneralHeaderConstraintsDataEntererAssignedEntityTELUse
	 */
	public boolean ValidateGeneralHeaderConstraintsDataEntererAssignedEntityTELUse(DiagnosticChain vb, DataElementLevel del)
	{
		if (del != null && del != DataElementLevel.DEL_CDA_HEADER)
		{
			return true;
		}
		boolean result = !(Set(self).anyMatch(i2083 -> i2083.isNullFlavorUndefined())) || Set(self.getUses()).count()==0 || One(Set(self.getUses()).count())<=1;
		if (!result && vb != null)
		{
			vb.add(new BasicDiagnostic(Diagnostic.ERROR, null, 0, "Error: USRealmHeader - 2.5.15.i.d.1 use\n\t\tConformance: SHOULD contain zero or one [0..1] @use (CONF:7996)\n\t\tAnalysis: n/a\n\t\tValidation message: n/a", new Object [] { self }));
		}
		return result;
	}
