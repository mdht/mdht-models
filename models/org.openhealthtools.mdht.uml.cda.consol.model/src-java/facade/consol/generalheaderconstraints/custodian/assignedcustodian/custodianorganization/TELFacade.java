package facade.consol.generalheaderconstraints.custodian.assignedcustodian.custodianorganization;
...
public class TELFacade extends facade.FacadeBase {
	...
	*/
	...
	{
			ValidateGeneralHeaderConstraintsCustodianAssignedCustodianCustodianOrganizationTELUse(vb, del);
	 ...
	 * Context: /GeneralHeaderConstraints/custodian/assignedCustodian/representedCustodianOrganization/telecom
	 * Context Class: consol::GeneralHeaderConstraints::Custodian::AssignedCustodian::CustodianOrganization::TEL
	 * Constraint Name: consol::GeneralHeaderConstraints::Custodian::AssignedCustodian::CustodianOrganization::TEL::GeneralHeaderConstraintsCustodianAssignedCustodianCustodianOrganizationTELUse
	 */
	public boolean ValidateGeneralHeaderConstraintsCustodianAssignedCustodianCustodianOrganizationTELUse(DiagnosticChain vb, DataElementLevel del)
	{
		if (del != null && del != DataElementLevel.DEL_CDA_HEADER)
		{
			return true;
		}
		boolean result = !(Set(self).anyMatch(i2205 -> i2205.isNullFlavorUndefined())) || Set(self.getUses()).count()==0 || One(Set(self.getUses()).count())<=1;
		if (!result && vb != null)
		{
			vb.add(new BasicDiagnostic(Diagnostic.ERROR, null, 0, "Error: USRealmHeader - 2.5.16.i.a.4.i use\n\t\tConformance: SHOULD contain zero or one [0..1] @use\n\t\tAnalysis: n/a\n\t\tValidation message: n/a", new Object [] { self }));
		}
		return result;
	}
