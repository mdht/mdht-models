...
public class AuthorFacade extends facade.FacadeBase {
	...
	*/
	...
	{
			ValidateGeneralHeaderConstraintsAuthorTime(vb, del);
			ValidateGeneralHeaderConstraintsAuthorAssignedAuthor(vb, del);
	 ...
	 * Context: /GeneralHeaderConstraints/author
	 * Context Class: consol::GeneralHeaderConstraints::Author
	 * Constraint Name: consol::GeneralHeaderConstraints::Author::GeneralHeaderConstraintsAuthorTime
	 */
	public boolean ValidateGeneralHeaderConstraintsAuthorTime(DiagnosticChain vb, DataElementLevel del)
	{
	 ...
	 * Context: /GeneralHeaderConstraints/author
	 * Context Class: consol::GeneralHeaderConstraints::Author
	 * Constraint Name: consol::GeneralHeaderConstraints::Author::GeneralHeaderConstraintsAuthorAssignedAuthor
	 */
	public boolean ValidateGeneralHeaderConstraintsAuthorAssignedAuthor(DiagnosticChain vb, DataElementLevel del)
	{
		if (del != null && del != DataElementLevel.DEL_CDA_HEADER)
		{
			return true;
		}
		boolean result = Set(self.getAssignedAuthor()).filter(i1863 -> !(Set(i1863).count()==0) && true).count()==1;
		if (!result && vb != null)
		{
			vb.add(new BasicDiagnostic(Diagnostic.ERROR, null, 0, "Error: USRealmHeader - 2.5.14.ii assignedAuthor\n\t\tConformance: SHALL contain exactly one [1..1] assignedAuthor (CONF:5448)\n\t\tAnalysis: n/a\n\t\tValidation message: n/a", new Object [] { self }));
		}
		return result;
	}
	...
	public Stream<facade.consol.generalheaderconstraints.author.AssignedAuthorFacade> assignedAuthor()
	{
		return Set(self.getAssignedAuthor()).filter( x -> facade.consol.generalheaderconstraints.author.AssignedAuthorFacade.isKindOf(x)).map( x -> new facade.consol.generalheaderconstraints.author.AssignedAuthorFacade(x));
	}

	public facade.consol.generalheaderconstraints.author.AssignedAuthorFacade GetOrCreateAssignedAuthor()
	{
		Optional<facade.consol.generalheaderconstraints.author.AssignedAuthorFacade> lastOrDefault = assignedAuthor().reduce((a, b) -> b);
		if (lastOrDefault.isPresent())
		{
	...
	public facade.consol.generalheaderconstraints.author.AssignedAuthorFacade CreateAnotherAssignedAuthor()
	{
		AssignedAuthor element = CDAFactory.eINSTANCE.createAssignedAuthor();
		facade.consol.generalheaderconstraints.author.AssignedAuthorFacade elementFacade = new facade.consol.generalheaderconstraints.author.AssignedAuthorFacade(element);
		elementFacade.Init();
		self.setAssignedAuthor(element);
		return elementFacade;
	}

	public facade.consol.generalheaderconstraints.author.AssignedAuthorFacade AssignedAuthor() {
