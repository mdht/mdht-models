package facade.consol.generalheaderconstraints.legalauthenticator.assignedentity;
...
public class TELFacade extends facade.FacadeBase {
	...
	*/
	...
	{
			ValidateGeneralHeaderConstraintsLegalAuthenticatorAssignedEntityTELUse(vb, del);
	 ...
	 * Context: /GeneralHeaderConstraints/legalAuthenticator/assignedEntity/telecom
	 * Context Class: consol::GeneralHeaderConstraints::LegalAuthenticator::AssignedEntity::TEL
	 * Constraint Name: consol::GeneralHeaderConstraints::LegalAuthenticator::AssignedEntity::TEL::GeneralHeaderConstraintsLegalAuthenticatorAssignedEntityTELUse
	 */
	public boolean ValidateGeneralHeaderConstraintsLegalAuthenticatorAssignedEntityTELUse(DiagnosticChain vb, DataElementLevel del)
	{
		if (del != null && del != DataElementLevel.DEL_CDA_HEADER)
		{
			return true;
		}
		boolean result = !(Set(self).anyMatch(i2407 -> i2407.isNullFlavorUndefined())) || Set(self.getUses()).count()==0 || One(Set(self.getUses()).count())<=1;
		if (!result && vb != null)
		{
			vb.add(new BasicDiagnostic(Diagnostic.ERROR, null, 0, "Error: USRealmHeader - 2.5.18.iii.d.1 use\n\t\tConformance: SHOULD contain zero or one [0..1] @use (CONF:7999)\n\t\tAnalysis: n/a\n\t\tValidation message: n/a", new Object [] { self }));
		}
		return result;
	}
