...
public class DocumentationOfFacade extends facade.FacadeBase {
	...
	*/
	...
	{
			ValidateGeneralHeaderConstraintsDocumentationOfServiceEvent(vb, del);
	 ...
	 * Context: /GeneralHeaderConstraints/documentationOf
	 * Context Class: consol::GeneralHeaderConstraints::DocumentationOf
	 * Constraint Name: consol::GeneralHeaderConstraints::DocumentationOf::GeneralHeaderConstraintsDocumentationOfServiceEvent
	 */
	public boolean ValidateGeneralHeaderConstraintsDocumentationOfServiceEvent(DiagnosticChain vb, DataElementLevel del)
	{
		if (del != null && del != DataElementLevel.DEL_CDA_HEADER)
		{
			return true;
		}
		boolean result = Set(self.getServiceEvent()).filter(i2761 -> !(Set(i2761).count()==0) && true).count()==1;
		if (!result && vb != null)
		{
			vb.add(new BasicDiagnostic(Diagnostic.ERROR, null, 0, "Error: USRealmHeader - 2.5.23.i serviceEvent\n\t\tConformance: SHALL contain exactly one [1..1] serviceEvent (CONF:14836)\n\t\tAnalysis: n/a\n\t\tValidation message: n/a", new Object [] { self }));
		}
		return result;
	}

	public Stream<facade.consol.generalheaderconstraints.documentationof.ServiceEventFacade> serviceEvent()
	{
		return Set(self.getServiceEvent()).filter( x -> facade.consol.generalheaderconstraints.documentationof.ServiceEventFacade.isKindOf(x)).map( x -> new facade.consol.generalheaderconstraints.documentationof.ServiceEventFacade(x));
	}

	public facade.consol.generalheaderconstraints.documentationof.ServiceEventFacade GetOrCreateServiceEvent()
	{
		Optional<facade.consol.generalheaderconstraints.documentationof.ServiceEventFacade> lastOrDefault = serviceEvent().reduce((a, b) -> b);
