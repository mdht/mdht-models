...
public class TELFacade extends facade.FacadeBase {
	...
	*/
	...
	{
			ValidateGeneralHeaderConstraintsAuthorAssignedAuthorTELUse(vb, del);
	 ...
	 * Context: /GeneralHeaderConstraints/author/assignedAuthor/telecom
	 * Context Class: consol::GeneralHeaderConstraints::Author::AssignedAuthor::TEL
	 * Constraint Name: consol::GeneralHeaderConstraints::Author::AssignedAuthor::TEL::GeneralHeaderConstraintsAuthorAssignedAuthorTELUse
	 */
	public boolean ValidateGeneralHeaderConstraintsAuthorAssignedAuthorTELUse(DiagnosticChain vb, DataElementLevel del)
	{
		if (del != null && del != DataElementLevel.DEL_CDA_HEADER)
		{
			return true;
		}
		boolean result = !(Set(self).anyMatch(i1933 -> i1933.isNullFlavorUndefined())) || Set(self.getUses()).count()==0 || One(Set(self.getUses()).count())<=1;
		if (!result && vb != null)
		{
			vb.add(new BasicDiagnostic(Diagnostic.ERROR, null, 0, "Error: USRealmHeader - 2.5.14.ii.d.1 use\n\t\tConformance: SHOULD contain zero or one [0..1] @use (CONF:7995)\n\t\tAnalysis: n/a\n\t\tValidation message: n/a", new Object [] { self }));
		}
		return result;
	}
