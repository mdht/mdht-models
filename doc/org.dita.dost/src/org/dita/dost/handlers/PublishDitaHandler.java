	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		if (event.getParameter("org.dita.dost.parameter.target") != null) {
			try {
				...
				// e.printStackTrace();
				// org.eclipse.swt.widgets.Shell theShell = window.getShell();
				// MessageDialog.openError(theShell, "DITA-OT Publishing", "Error" + e.getMessage());
				// ;
			}
		} else {
			// IWorkbenchWindow window = HandlerUtil.getActiveWorkbenchWindowChecked(event);
			// Shell theShell = window.getShell();
			// MessageDialog.openError(theShell, "DITA-OT Publishing", "No Publishing Target");
			// ;
		}
