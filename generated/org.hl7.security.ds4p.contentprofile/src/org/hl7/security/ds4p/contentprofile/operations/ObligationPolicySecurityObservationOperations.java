...
"value.code = 'SECCONOBS' and value.codeSystem = '2.16.840.1.113883.1.11.20457'";
    ...
    DatatypesUtil.increment(context,"ObligationPolicySecurityObservationSecurityObservationValueP","ERROR");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_SECURITY_OBSERVATION_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(obligationPolicySecurityObservation)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 CONTENTPROFILEValidator.DIAGNOSTIC_SOURCE,
						 CONTENTPROFILEValidator.OBLIGATION_POLICY_SECURITY_OBSERVATION__SECURITY_OBSERVATION_VALUE_P,
						 CONTENTPROFILEPlugin.INSTANCE.getString("ObligationPolicySecurityObservationSecurityObservationValueP"),
						 new Object [] { obligationPolicySecurityObservation }));
			}
...
"value.code = 'CPLYCD' or value.code = 'ENCRYPT')";
    ...
    DatatypesUtil.increment(context,"ObligationPolicySecurityObservationSecurityObservationValue","WARNING");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_SECURITY_OBSERVATION_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(obligationPolicySecurityObservation)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.WARNING,
						 CONTENTPROFILEValidator.DIAGNOSTIC_SOURCE,
						 CONTENTPROFILEValidator.OBLIGATION_POLICY_SECURITY_OBSERVATION__SECURITY_OBSERVATION_VALUE,
						 CONTENTPROFILEPlugin.INSTANCE.getString("ObligationPolicySecurityObservationSecurityObservationValue"),
						 new Object [] { obligationPolicySecurityObservation }));
			}
