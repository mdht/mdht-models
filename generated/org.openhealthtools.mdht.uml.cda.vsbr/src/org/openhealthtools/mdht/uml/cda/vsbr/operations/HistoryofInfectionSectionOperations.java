...
public class HistoryofInfectionSectionOperations extends SectionOperations {
	...
	public static boolean validateHistoryofInfectionSectionTemplateId(
		...
		if (!EOCL_ENV.createQuery(VALIDATE_HISTORYOF_INFECTION_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString(
						...
						new Object[] {
								"HistoryofInfectionSectionTemplateId",
								org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(
									historyofInfectionSection, context) }), new Object[] { historyofInfectionSection }));
			}
	...
	public static boolean validateHistoryofInfectionSectionText(HistoryofInfectionSection historyofInfectionSection,
		...
		if (!EOCL_ENV.createQuery(VALIDATE_HISTORYOF_INFECTION_SECTION_TEXT__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			historyofInfectionSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, VsbrValidator.DIAGNOSTIC_SOURCE,
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_HISTORYOF_INFECTION_SECTION_INFECTIONS_PRESENT__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.entry->exists(entry : cda::Entry | not entry.observation.oclIsUndefined() and entry.observation.oclIsKindOf(vsbr::InfectionPresent) and entry.typeCode = vocab::x_ActRelationshipEntry::COMP)";
	...
	public static boolean validateHistoryofInfectionSectionInfectionsPresent(
		...
		if (!EOCL_ENV.createQuery(
			VALIDATE_HISTORYOF_INFECTION_SECTION_INFECTIONS_PRESENT__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			historyofInfectionSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, VsbrValidator.DIAGNOSTIC_SOURCE,
					...
					org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString(
						"_UI_GenericInvariant_diagnostic",
						new Object[] {
								"HistoryofInfectionSectionInfectionsPresent",
								org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(
									historyofInfectionSection, context) }), new Object[] { historyofInfectionSection }));
			}
	 ...
	 * @ordered
	 */
	protected static final String GET_INFECTIONS_PRESENTS__EOCL_EXP = "self.getObservations()->select(observation : cda::Observation | not observation.oclIsUndefined() and observation.oclIsKindOf(vsbr::InfectionPresent)).oclAsType(vsbr::InfectionPresent)";
	...
	public static EList<InfectionPresent> getInfectionsPresents(HistoryofInfectionSection historyofInfectionSection) {
		if (GET_INFECTIONS_PRESENTS__EOCL_QRY == null) {
			...
			helper.setOperationContext(
				VsbrPackage.Literals.HISTORYOF_INFECTION_SECTION,
				VsbrPackage.Literals.HISTORYOF_INFECTION_SECTION.getEAllOperations().get(58));
			try {
