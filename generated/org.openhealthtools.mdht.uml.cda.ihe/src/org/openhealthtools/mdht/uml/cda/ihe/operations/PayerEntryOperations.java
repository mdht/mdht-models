...
public class PayerEntryOperations extends PolicyActivityOperations {
	...
	public static boolean validatePolicyActivityTemplateId(PayerEntry payerEntry, DiagnosticChain diagnostics,
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_POLICY_ACTIVITY_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(payerEntry)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, IHEValidator.DIAGNOSTIC_SOURCE,
						IHEValidator.PAYER_ENTRY__POLICY_ACTIVITY_TEMPLATE_ID,
						IHEPlugin.INSTANCE.getString("PayerEntryPolicyActivityTemplateId"),
