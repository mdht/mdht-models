...
public class HospitalDischargePhysicalOperations extends SectionOperations {
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_HOSPITAL_DISCHARGE_PHYSICAL_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(hospitalDischargePhysical))
    {
      if (diagnostics != null)
      {
        diagnostics.add
...
"value.code = '10184-0' and value.codeSystem = '2.16.840.1.113883.6.1')";
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_HOSPITAL_DISCHARGE_PHYSICAL_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(hospitalDischargePhysical))
    {
      if (diagnostics != null)
      {
        diagnostics.add
