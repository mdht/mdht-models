...
public class CombinationMedicationOperations extends MedicationOperations {
	...
	protected static ThreadLocal<Constraint> VALIDATE_COMBINATION_MEDICATION_SUBSTANCE_ADMINISTRATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<Constraint>();
	...
	public static boolean validateCombinationMedicationSubstanceAdministration(
		...
		if (VALIDATE_COMBINATION_MEDICATION_SUBSTANCE_ADMINISTRATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {

			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(IHEPackage.Literals.COMBINATION_MEDICATION);
			try {
				VALIDATE_COMBINATION_MEDICATION_SUBSTANCE_ADMINISTRATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(
						VALIDATE_COMBINATION_MEDICATION_SUBSTANCE_ADMINISTRATION__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_COMBINATION_MEDICATION_SUBSTANCE_ADMINISTRATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
				...
				diagnostics.add(
					new BasicDiagnostic(
						...
						org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString(
							...
							new Object[] {
									"CombinationMedicationCombinationMedicationSubstanceAdministration",
									org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(
										combinationMedication, context) }),
	 ...
	 * @ordered
	 */
	protected static ThreadLocal<Constraint> VALIDATE_COMBINATION_MEDICATION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<Constraint>();
	...
	public static boolean validateCombinationMedicationTemplateId(CombinationMedication combinationMedication,
		...
		if (VALIDATE_COMBINATION_MEDICATION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {

			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(IHEPackage.Literals.COMBINATION_MEDICATION);
			try {
				VALIDATE_COMBINATION_MEDICATION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(
						VALIDATE_COMBINATION_MEDICATION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_COMBINATION_MEDICATION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
				combinationMedication)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						...
						IHEPlugin.INSTANCE.getString("CombinationMedicationCombinationMedicationTemplateId"),
						new Object[] { combinationMedication }));
			}
