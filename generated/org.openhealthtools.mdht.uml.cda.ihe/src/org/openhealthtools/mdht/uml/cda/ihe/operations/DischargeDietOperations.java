...
public class DischargeDietOperations extends SectionOperations {
	 ...
	 * @ordered
	 */
	protected static ThreadLocal<Constraint> VALIDATE_DISCHARGE_DIET_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<Constraint>();
	...
	public static boolean validateDischargeDietTemplateId(DischargeDiet dischargeDiet, DiagnosticChain diagnostics,
		...
		if (VALIDATE_DISCHARGE_DIET_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {

			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(IHEPackage.Literals.DISCHARGE_DIET);
			try {
				VALIDATE_DISCHARGE_DIET_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_DISCHARGE_DIET_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_DISCHARGE_DIET_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(dischargeDiet)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, IHEValidator.DIAGNOSTIC_SOURCE,
						IHEValidator.DISCHARGE_DIET__DISCHARGE_DIET_TEMPLATE_ID,
						IHEPlugin.INSTANCE.getString("DischargeDietDischargeDietTemplateId"),
	 ...
	 * @ordered
	 */
	protected static ThreadLocal<Constraint> VALIDATE_DISCHARGE_DIET_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<Constraint>();
	...
	public static boolean validateDischargeDietCode(DischargeDiet dischargeDiet, DiagnosticChain diagnostics,
		...
		if (VALIDATE_DISCHARGE_DIET_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {

			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(IHEPackage.Literals.DISCHARGE_DIET);
			try {
				VALIDATE_DISCHARGE_DIET_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_DISCHARGE_DIET_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}

		if (!EOCL_ENV.get().createQuery(VALIDATE_DISCHARGE_DIET_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
			dischargeDiet)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, IHEValidator.DIAGNOSTIC_SOURCE,
						IHEValidator.DISCHARGE_DIET__DISCHARGE_DIET_CODE,
						IHEPlugin.INSTANCE.getString("DischargeDietDischargeDietCode"),
						new Object[] { dischargeDiet }));
			}
