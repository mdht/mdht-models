...
public class CommentOperations extends org.openhealthtools.mdht.uml.cda.ccd.operations.CommentOperations {
	 ...
	 * @generated
	 */
	public static  boolean validateIHECommentEffectiveTimeWhenAuthorPresent(Comment comment, DiagnosticChain diagnostics, Map<Object, Object> context) {
		...
		if (VALIDATE_IHE_COMMENT_EFFECTIVE_TIME_WHEN_AUTHOR_PRESENT__DIAGNOSTIC_CHAIN_MAP__EOCL_INV == null) {
			...
			if (!EOCL_ENV.createQuery(VALIDATE_IHE_COMMENT_EFFECTIVE_TIME_WHEN_AUTHOR_PRESENT__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(comment)) {
				if (diagnostics != null) {
				...
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 ...
						 IHEPlugin.INSTANCE.getString("IHECommentEffectiveTimeWhenAuthorPresent"),
						 new Object [] { comment }));
				}
			return false;
		}		
	 ...
	 * @generated
	 */
	public static  boolean validateIHECommentHasAuthorId(Comment comment, DiagnosticChain diagnostics, Map<Object, Object> context) {
				...
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 ...
						 IHEPlugin.INSTANCE.getString("IHECommentHasAuthorId"),
						 new Object [] { comment }));
				}
			return false;
		}		
	...
	//( ((not self.author->isEmpty()) implies (  self.author.assignedAuthor.assignedPerson.name ->size() > 0 )) or  ((not self.author->isEmpty()) implies (  self.author.assignedAuthor.representedOrganization.name ->size() > 0)) )";
	//not self.author->isEmpty() implies ((  (not self.author.assignedAuthor.representedOrganization.name->isEmpty()) or (not self.author.assignedAuthor.assignedPerson.name->isEmpty())  ))";
	 ...
	 * @generated
	 */
	public static  boolean validateIHECommentHasAuthorOrganization(Comment comment, DiagnosticChain diagnostics, Map<Object, Object> context) {
		...
		if (VALIDATE_IHE_COMMENT_HAS_AUTHOR_ORGANIZATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV == null) {
			...
			if (!EOCL_ENV.createQuery(VALIDATE_IHE_COMMENT_HAS_AUTHOR_ORGANIZATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(comment)) {
				if (diagnostics != null) {
				...
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 ...
						 IHEPlugin.INSTANCE.getString("IHECommentHasAuthorOrganization"),
						 new Object [] { comment }));
				}
			return false;
		}		
