...
public class CodedFamilyMedicalHistorySectionOperations extends FamilyMedicalHistorySectionOperations {
	 ...
	 * @ordered
	 */
	protected static ThreadLocal<Constraint> VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<Constraint>();
	...
	public static boolean validateCodedFamilyMedicalHistorySectionTemplateId(
		...
		if (VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {

			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(IHEPackage.Literals.CODED_FAMILY_MEDICAL_HISTORY_SECTION);
			try {
				VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(
						VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
				...
				diagnostics.add(
					new BasicDiagnostic(
						...
						IHEPlugin.INSTANCE.getString(
							"CodedFamilyMedicalHistorySectionCodedFamilyMedicalHistorySectionTemplateId"),
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_FAMILY_HISTORY_ORGANIZER__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.nullFlavor <> vocab::NullFlavor::NI implies entry->one(entry : cda::Entry | not entry.organizer.oclIsUndefined() and entry.organizer.oclIsKindOf(ihe::FamilyHistoryOrganizer))";
	 ...
	 * @ordered
	 */
	protected static ThreadLocal<Constraint> VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_FAMILY_HISTORY_ORGANIZER__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<Constraint>();
	...
	public static boolean validateCodedFamilyMedicalHistorySectionFamilyHistoryOrganizer(
		...
		if (VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_FAMILY_HISTORY_ORGANIZER__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {

			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(IHEPackage.Literals.CODED_FAMILY_MEDICAL_HISTORY_SECTION);
			try {
				VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_FAMILY_HISTORY_ORGANIZER__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(
						VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_FAMILY_HISTORY_ORGANIZER__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_CODED_FAMILY_MEDICAL_HISTORY_SECTION_FAMILY_HISTORY_ORGANIZER__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
				codedFamilyMedicalHistorySection)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						...
						IHEPlugin.INSTANCE.getString(
							"CodedFamilyMedicalHistorySectionCodedFamilyMedicalHistorySectionFamilyHistoryOrganizer"),
						new Object[] { codedFamilyMedicalHistorySection }));
			}
	...
	public static FamilyHistoryOrganizer getIHEFamilyHistoryOrganizer(
		...
		if (GET_IHE_FAMILY_HISTORY_ORGANIZER__EOCL_QRY == null) {
			...
			try {
				GET_IHE_FAMILY_HISTORY_ORGANIZER__EOCL_QRY = helper.createQuery(
					GET_IHE_FAMILY_HISTORY_ORGANIZER__EOCL_EXP);
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}

		OCL.Query query = EOCL_ENV.get().createQuery(GET_IHE_FAMILY_HISTORY_ORGANIZER__EOCL_QRY);
