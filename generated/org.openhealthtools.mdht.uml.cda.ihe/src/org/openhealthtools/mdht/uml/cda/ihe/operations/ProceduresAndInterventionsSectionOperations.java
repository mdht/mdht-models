...
public class ProceduresAndInterventionsSectionOperations extends SectionOperations {
	...
	protected static Constraint VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV;

	/**
	 * <!-- begin-user-doc -->
	 * <!-- end-user-doc -->
	 * <!-- begin-model-doc -->
	 * self.templateId->exists(id : datatypes::II | id.root = '1.3.6.1.4.1.19376.1.5.3.1.1.13.2.11')
	...
	public static boolean validateProceduresAndInterventionsSectionTemplateId(
		...
		if (VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setContext(IHEPackage.Literals.PROCEDURES_AND_INTERVENTIONS_SECTION);
			try {
				VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = helper.createInvariant(VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP);
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		if (!EOCL_ENV.createQuery(
			VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					IHEPlugin.INSTANCE.getString("ProceduresAndInterventionsSectionTemplateId"),
	...
	protected static Constraint VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV;
	...
	public static boolean validateProceduresAndInterventionsSectionCode(
		...
		if (VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setContext(IHEPackage.Literals.PROCEDURES_AND_INTERVENTIONS_SECTION);
			try {
				VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = helper.createInvariant(VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP);
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		if (!EOCL_ENV.createQuery(VALIDATE_PROCEDURES_AND_INTERVENTIONS_SECTION_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			proceduresAndInterventionsSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, IHEValidator.DIAGNOSTIC_SOURCE,
					IHEValidator.PROCEDURES_AND_INTERVENTIONS_SECTION__PROCEDURES_AND_INTERVENTIONS_SECTION_CODE,
					IHEPlugin.INSTANCE.getString("ProceduresAndInterventionsSectionCode"),
					new Object[] { proceduresAndInterventionsSection }));
			}
	...
	public static ProcedureEntry getProcedureEntry(ProceduresAndInterventionsSection proceduresAndInterventionsSection) {
