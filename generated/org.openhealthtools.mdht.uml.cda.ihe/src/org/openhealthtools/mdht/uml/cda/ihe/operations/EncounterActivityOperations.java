...
public class EncounterActivityOperations extends EncounterEntryOperations {
	...
	public static boolean validateEncountersActivityTemplateId(EncounterActivity encounterActivity,
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_ENCOUNTERS_ACTIVITY_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(encounterActivity)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, IHEValidator.DIAGNOSTIC_SOURCE,
						IHEValidator.ENCOUNTER_ACTIVITY__ENCOUNTERS_ACTIVITY_TEMPLATE_ID,
						IHEPlugin.INSTANCE.getString("EncounterActivityEncountersActivityTemplateId"),
