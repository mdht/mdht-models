...
public class EncounterDiagnosis2Operations extends EncounterDiagnosisOperations {
    ...
    DatatypesUtil.increment(context,"EncounterDiagnosis2EncounterDiagnosis2TemplateId","ERROR");
		...
		if (VALIDATE_ENCOUNTER_DIAGNOSIS2_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			try {
				VALIDATE_ENCOUNTER_DIAGNOSIS2_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_ENCOUNTER_DIAGNOSIS2_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_ENCOUNTER_DIAGNOSIS2_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(encounterDiagnosis2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
	...
	public static EList<ProblemObservation2> getConsolProblemObservation2s(EncounterDiagnosis2 encounterDiagnosis2) {
		...
		if (GET_CONSOL_PROBLEM_OBSERVATION2S__EOCL_QRY == null) {
			...
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setOperationContext(ConsolPackage.Literals.ENCOUNTER_DIAGNOSIS2, ConsolPackage.Literals.ENCOUNTER_DIAGNOSIS2.getEAllOperations().get(59));
			try {
    ...
    DatatypesUtil.increment(context,"EncounterDiagnosis2EncounterDiagnosisCodeP","ERROR");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_ENCOUNTER_DIAGNOSIS_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(encounterDiagnosis2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
...
"value.code = '29308-4' and value.codeSystem = '2.16.840.1.113883.6.1')";
    ...
    DatatypesUtil.increment(context,"EncounterDiagnosis2EncounterDiagnosisCode","ERROR");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_ENCOUNTER_DIAGNOSIS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(encounterDiagnosis2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
    ...
    DatatypesUtil.increment(context,"EncounterDiagnosis2EncounterDiagnosisProblemObservation","ERROR");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_ENCOUNTER_DIAGNOSIS_PROBLEM_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(encounterDiagnosis2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
