...
public class DischargeDiagnosisSection2Operations extends HospitalDischargeDiagnosisSectionOperations {
    ...
    DatatypesUtil.increment(context,"DischargeDiagnosisSection2DischargeDiagnosisSection2TemplateId","ERROR");
		...
		if (VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			try {
				VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(dischargeDiagnosisSection2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 ...
						 ConsolPlugin.INSTANCE.getString("DischargeDiagnosisSection2DischargeDiagnosisSection2TemplateId"),
...
"value.code = '78375-3' and value.codeSystem = '2.16.840.1.113883.6.1')) )))";
    ...
    DatatypesUtil.increment(context,"DischargeDiagnosisSection2DischargeDiagnosisSection2CETranslation","ERROR");
		...
		if (VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_CE_TRANSLATION__DIAGNOSTIC_CHAIN_MAP__EOCL_QRY.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.DISCHARGE_DIAGNOSIS_SECTION2);
			try {
				OCLExpression<EClassifier> oclExpression = helper.createQuery(VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_CE_TRANSLATION__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP);
				VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_CE_TRANSLATION__DIAGNOSTIC_CHAIN_MAP__EOCL_QRY.set(EOCL_ENV.get().createQuery(oclExpression));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		Object oclResult = VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_CE_TRANSLATION__DIAGNOSTIC_CHAIN_MAP__EOCL_QRY.get().evaluate(dischargeDiagnosisSection2);		
				...
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 ...
						 ConsolPlugin.INSTANCE.getString("DischargeDiagnosisSection2DischargeDiagnosisSection2CETranslation"),
						 new Object [] { eObject }));
				}
    ...
    DatatypesUtil.increment(context,"DischargeDiagnosisSection2DischargeDiagnosisSection2CETranslationP","ERROR");
		...
		if (VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_CE_TRANSLATION_P__DIAGNOSTIC_CHAIN_MAP__EOCL_QRY.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.DISCHARGE_DIAGNOSIS_SECTION2);
			try {
				OCLExpression<EClassifier> oclExpression = helper.createQuery(VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_CE_TRANSLATION_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP);
				VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_CE_TRANSLATION_P__DIAGNOSTIC_CHAIN_MAP__EOCL_QRY.set(EOCL_ENV.get().createQuery(oclExpression));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		Object oclResult = VALIDATE_DISCHARGE_DIAGNOSIS_SECTION2_CE_TRANSLATION_P__DIAGNOSTIC_CHAIN_MAP__EOCL_QRY.get().evaluate(dischargeDiagnosisSection2);		
		if (oclResult != null && oclResult instanceof Collection) {
			Collection<? extends EObject> oclResultSet = (Collection<? extends EObject>) oclResult;
	...
	public static HospitalDischargeDiagnosis2 getConsolHospitalDischargeDiagnosis2(
		...
		if (GET_CONSOL_HOSPITAL_DISCHARGE_DIAGNOSIS2__EOCL_QRY == null) {
			...
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setOperationContext(ConsolPackage.Literals.DISCHARGE_DIAGNOSIS_SECTION2, ConsolPackage.Literals.DISCHARGE_DIAGNOSIS_SECTION2.getEAllOperations().get(65));
			try {
    ...
    DatatypesUtil.increment(context,"DischargeDiagnosisSection2HospitalDischargeDiagnosisSectionCodeP","ERROR");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_HOSPITAL_DISCHARGE_DIAGNOSIS_SECTION_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(dischargeDiagnosisSection2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
			...
			if (context != null) {
				...
				if (passToken == null) {
					...
					context.put("org.openhealthtools.mdht.uml.cda.consol.HospitalDischargeDiagnosisSectionCodeP", passToken);
				}
...
"value.code = '11535-2' and value.codeSystem = '2.16.840.1.113883.6.1')";
	...
	public static boolean validateHospitalDischargeDiagnosisSectionCode(
		...
		Object passToken = (context == null) ? null : context.get("org.openhealthtools.mdht.uml.cda.consol.HospitalDischargeDiagnosisSectionCodeP");
    ...
    DatatypesUtil.increment(context,"DischargeDiagnosisSection2HospitalDischargeDiagnosisSectionHospitalDischargeDiagnosis","WARNING");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_HOSPITAL_DISCHARGE_DIAGNOSIS_SECTION_HOSPITAL_DISCHARGE_DIAGNOSIS__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(dischargeDiagnosisSection2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.WARNING,
