...
public class VitalSignsSectionEntriesOptional2Operations extends VitalSignsSectionEntriesOptionalOperations {
	...
	public static EList<VitalSignsOrganizer2> getConsolVitalSignsOrganizer2s(
		...
		if (GET_CONSOL_VITAL_SIGNS_ORGANIZER2S__EOCL_QRY == null) {
			...
			helper.setOperationContext(ConsolPackage.Literals.VITAL_SIGNS_SECTION_ENTRIES_OPTIONAL2, ConsolPackage.Literals.VITAL_SIGNS_SECTION_ENTRIES_OPTIONAL2.getEAllOperations().get(62));
			try {
    ...
    DatatypesUtil.increment(context,"VitalSignsSectionEntriesOptional2VitalSignsSectionEntriesOptionalTemplateId","ERROR");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_VITAL_SIGNS_SECTION_ENTRIES_OPTIONAL_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(vitalSignsSectionEntriesOptional2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
...
"value.code = '8716-3' and value.codeSystem = '2.16.840.1.113883.6.1')";
    ...
    DatatypesUtil.increment(context,"VitalSignsSectionEntriesOptional2VitalSignsSectionEntriesOptionalCode","ERROR");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_VITAL_SIGNS_SECTION_ENTRIES_OPTIONAL_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(vitalSignsSectionEntriesOptional2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
    ...
    DatatypesUtil.increment(context,"VitalSignsSectionEntriesOptional2VitalSignsSectionEntriesOptionalVitalSignsOrganizer","WARNING");
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_VITAL_SIGNS_SECTION_ENTRIES_OPTIONAL_VITAL_SIGNS_ORGANIZER__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(vitalSignsSectionEntriesOptional2)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.WARNING,
						 ...
						 ConsolPlugin.INSTANCE.getString("VitalSignsSectionEntriesOptional2VitalSignsSectionEntriesOptionalVitalSignsOrganizer"),
						 new Object [] { vitalSignsSectionEntriesOptional2 }));
			}
