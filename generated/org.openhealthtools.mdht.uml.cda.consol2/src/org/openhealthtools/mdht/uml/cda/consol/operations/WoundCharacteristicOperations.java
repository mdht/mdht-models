...
public class WoundCharacteristicOperations extends ClinicalStatementOperations {
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicTemplateId(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			try {
				VALIDATE_WOUND_CHARACTERISTIC_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_WOUND_CHARACTERISTIC_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_WOUND_CHARACTERISTIC_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
				woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicClassCode(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_WOUND_CHARACTERISTIC_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_WOUND_CHARACTERISTIC_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
				woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicMoodCode(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_WOUND_CHARACTERISTIC_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_WOUND_CHARACTERISTIC_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicId(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_WOUND_CHARACTERISTIC_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}

		if (!EOCL_ENV.get().createQuery(VALIDATE_WOUND_CHARACTERISTIC_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
			woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicCodeP(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_WOUND_CHARACTERISTIC_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_WOUND_CHARACTERISTIC_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicCode(WoundCharacteristic woundCharacteristic,
		...
		Object passToken = (context == null)
				? null
				: context.get("org.openhealthtools.mdht.uml.cda.consol.WoundCharacteristicCodeP");
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_WOUND_CHARACTERISTIC_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}

		if (!EOCL_ENV.get().createQuery(VALIDATE_WOUND_CHARACTERISTIC_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
			woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_STATUS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicStatusCode(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_STATUS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_STATUS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_WOUND_CHARACTERISTIC_STATUS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_WOUND_CHARACTERISTIC_STATUS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
				woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_STATUS_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicStatusCodeP(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_STATUS_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_STATUS_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_WOUND_CHARACTERISTIC_STATUS_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
				woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_EFFECTIVE_TIME__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicEffectiveTime(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_EFFECTIVE_TIME__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_EFFECTIVE_TIME__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_WOUND_CHARACTERISTIC_EFFECTIVE_TIME__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
				woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicValue(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_WOUND_CHARACTERISTIC_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_WOUND_CHARACTERISTIC_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
	...
	protected static ThreadLocal<Constraint> VALIDATE_WOUND_CHARACTERISTIC_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new ThreadLocal<>();
	...
	public static boolean validateWoundCharacteristicValueP(WoundCharacteristic woundCharacteristic,
		...
		if (VALIDATE_WOUND_CHARACTERISTIC_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(ConsolPackage.Literals.WOUND_CHARACTERISTIC);
			try {
				VALIDATE_WOUND_CHARACTERISTIC_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_WOUND_CHARACTERISTIC_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
		...
		if (!EOCL_ENV.get().createQuery(
			VALIDATE_WOUND_CHARACTERISTIC_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(woundCharacteristic)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
