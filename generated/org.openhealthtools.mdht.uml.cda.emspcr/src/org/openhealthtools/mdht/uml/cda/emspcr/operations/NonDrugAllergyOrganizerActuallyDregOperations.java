 ...
 * @generated
 */
public class NonDrugAllergyOrganizerActuallyDregOperations extends org.eclipse.mdht.uml.cda.operations.OrganizerOperations {
	...
	protected static ThreadLocal<  Constraint> VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EMS_DRUG_ALLERGY__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
	...
	public static boolean validateNonDrugAllergyOrganizerActuallyDregEMSDrugAllergy(
		...
		if (VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EMS_DRUG_ALLERGY__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG);
			try {
				VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EMS_DRUG_ALLERGY__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EMS_DRUG_ALLERGY__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EMS_DRUG_ALLERGY__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(nonDrugAllergyOrganizerActuallyDreg)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.INFO,
						 ...
						 EmspcrPlugin.INSTANCE.getString("NonDrugAllergyOrganizerActuallyDregNonDrugAllergyOrganizerActuallyDregEMSDrugAllergy"),
	...
	protected static ThreadLocal<  Constraint> VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EXISTENCE_OF_DRUG_ALLERGY_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
	...
	public static boolean validateNonDrugAllergyOrganizerActuallyDregExistenceOfDrugAllergyObservation(
		...
		if (VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EXISTENCE_OF_DRUG_ALLERGY_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG);
			try {
				VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EXISTENCE_OF_DRUG_ALLERGY_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EXISTENCE_OF_DRUG_ALLERGY_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG_EXISTENCE_OF_DRUG_ALLERGY_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(nonDrugAllergyOrganizerActuallyDreg)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
						 ...
						 EmspcrPlugin.INSTANCE.getString("NonDrugAllergyOrganizerActuallyDregNonDrugAllergyOrganizerActuallyDregExistenceOfDrugAllergyObservation"),
						 new Object [] { nonDrugAllergyOrganizerActuallyDreg }));
			}
	...
	public static EList<EMSDrugAllergy> getEMSDrugAllergies(
		...
		if (GET_EMS_DRUG_ALLERGIES__EOCL_QRY == null) {
			...
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setOperationContext(EmspcrPackage.Literals.NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG, EmspcrPackage.Literals.NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG.getEAllOperations().get(54));
			try {
	...
	public static ExistenceOfDrugAllergyObservation getExistenceOfDrugAllergyObservation(
		...
		if (GET_EXISTENCE_OF_DRUG_ALLERGY_OBSERVATION__EOCL_QRY == null) {
			...
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setOperationContext(EmspcrPackage.Literals.NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG, EmspcrPackage.Literals.NON_DRUG_ALLERGY_ORGANIZER_ACTUALLY_DREG.getEAllOperations().get(55));
			try {
