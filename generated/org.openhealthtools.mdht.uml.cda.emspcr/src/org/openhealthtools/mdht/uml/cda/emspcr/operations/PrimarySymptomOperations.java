 ...
 * @generated
 */
public class PrimarySymptomOperations extends org.eclipse.mdht.uml.cda.operations.ClinicalStatementOperations {
	...
	protected static ThreadLocal<  Constraint> VALIDATE_PRIMARY_SYMPTOM_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
	...
	public static boolean validatePrimarySymptomTemplateId(PrimarySymptom primarySymptom, DiagnosticChain diagnostics,
		...
		if (VALIDATE_PRIMARY_SYMPTOM_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.PRIMARY_SYMPTOM);
			try {
				VALIDATE_PRIMARY_SYMPTOM_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_PRIMARY_SYMPTOM_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_PRIMARY_SYMPTOM_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(primarySymptom)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
	...
	protected static ThreadLocal<  Constraint> VALIDATE_PRIMARY_SYMPTOM_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
	...
	public static boolean validatePrimarySymptomMoodCode(PrimarySymptom primarySymptom, DiagnosticChain diagnostics,
		...
		if (VALIDATE_PRIMARY_SYMPTOM_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.PRIMARY_SYMPTOM);
			try {
				VALIDATE_PRIMARY_SYMPTOM_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_PRIMARY_SYMPTOM_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_PRIMARY_SYMPTOM_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(primarySymptom)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
	...
	protected static ThreadLocal<  Constraint> VALIDATE_PRIMARY_SYMPTOM_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
	...
	public static boolean validatePrimarySymptomCodeP(PrimarySymptom primarySymptom, DiagnosticChain diagnostics,
		...
		if (VALIDATE_PRIMARY_SYMPTOM_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.PRIMARY_SYMPTOM);
			try {
				VALIDATE_PRIMARY_SYMPTOM_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_PRIMARY_SYMPTOM_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_PRIMARY_SYMPTOM_CODE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(primarySymptom)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
...
"value.code = '67774-0' and value.codeSystem = '2.16.840.1.113883.6.1')";
	...
	protected static ThreadLocal<  Constraint> VALIDATE_PRIMARY_SYMPTOM_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
	...
	public static boolean validatePrimarySymptomCode(PrimarySymptom primarySymptom, DiagnosticChain diagnostics,
		...
		if (VALIDATE_PRIMARY_SYMPTOM_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.PRIMARY_SYMPTOM);
			try {
				VALIDATE_PRIMARY_SYMPTOM_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_PRIMARY_SYMPTOM_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_PRIMARY_SYMPTOM_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(primarySymptom)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
...
"value.codeSystem = '2.16.840.1.113883.6.90'))";
	...
	protected static ThreadLocal<  Constraint> VALIDATE_PRIMARY_SYMPTOM_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
	...
	public static boolean validatePrimarySymptomValue(PrimarySymptom primarySymptom, DiagnosticChain diagnostics,
		...
		if (VALIDATE_PRIMARY_SYMPTOM_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.PRIMARY_SYMPTOM);
			try {
				VALIDATE_PRIMARY_SYMPTOM_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_PRIMARY_SYMPTOM_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_PRIMARY_SYMPTOM_VALUE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(primarySymptom)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
	...
	protected static ThreadLocal<  Constraint> VALIDATE_PRIMARY_SYMPTOM_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = new  ThreadLocal<  Constraint>();
	...
	public static boolean validatePrimarySymptomValueP(PrimarySymptom primarySymptom, DiagnosticChain diagnostics,
		...
		if (VALIDATE_PRIMARY_SYMPTOM_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
		
	 
			OCL.Helper helper = EOCL_ENV.get().createOCLHelper();
			helper.setContext(EmspcrPackage.Literals.PRIMARY_SYMPTOM);
			try {
				VALIDATE_PRIMARY_SYMPTOM_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(helper.createInvariant(VALIDATE_PRIMARY_SYMPTOM_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			}
			catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		...
		if (!EOCL_ENV.get().createQuery(VALIDATE_PRIMARY_SYMPTOM_VALUE_P__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(primarySymptom)) {
			if (diagnostics != null) {
				diagnostics.add
					(new BasicDiagnostic
						(Diagnostic.ERROR,
