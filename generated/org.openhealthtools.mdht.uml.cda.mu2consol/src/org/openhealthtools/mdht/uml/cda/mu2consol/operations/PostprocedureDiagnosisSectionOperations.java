...
public class PostprocedureDiagnosisSectionOperations
	 ...
	 * {@link #validateMu2consolPostprocedureDiagnosisSectionPostprocedureDiagnosis(PostprocedureDiagnosisSection, org.eclipse.emf.common.util.DiagnosticChain, java.util.Map)
	 * <em>Validate Mu2consol Postprocedure Diagnosis Section Postprocedure Diagnosis</em>}
	 * ' operation. <!-- begin-user-doc --> <!-- end-user-doc -->
	 * 
	 * @see #validateMu2consolPostprocedureDiagnosisSectionPostprocedureDiagnosis(PostprocedureDiagnosisSection,
	...
	public static boolean validateMu2consolPostprocedureDiagnosisSectionPostprocedureDiagnosis(
		...
		if (VALIDATE_MU2CONSOL_POSTPROCEDURE_DIAGNOSIS_SECTION_POSTPROCEDURE_DIAGNOSIS__DIAGNOSTIC_CHAIN_MAP__EOCL_INV == null) {
			...
			try {
				VALIDATE_MU2CONSOL_POSTPROCEDURE_DIAGNOSIS_SECTION_POSTPROCEDURE_DIAGNOSIS__DIAGNOSTIC_CHAIN_MAP__EOCL_INV = helper
						.createInvariant(VALIDATE_MU2CONSOL_POSTPROCEDURE_DIAGNOSIS_SECTION_POSTPROCEDURE_DIAGNOSIS__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP);
		...
		if (!EOCL_ENV
				.createQuery(
						VALIDATE_MU2CONSOL_POSTPROCEDURE_DIAGNOSIS_SECTION_POSTPROCEDURE_DIAGNOSIS__DIAGNOSTIC_CHAIN_MAP__EOCL_INV)
				.check(postprocedureDiagnosisSection)) {
			if (diagnostics != null) {
				diagnostics
	...
	public static PostprocedureDiagnosis getmu2consolPostprocedureDiagnosis(
		...
		if (GETMU2CONSOL_POSTPROCEDURE_DIAGNOSIS__EOCL_QRY == null) {
			...
			helper.setOperationContext(
					Mu2consolPackage.Literals.POSTPROCEDURE_DIAGNOSIS_SECTION,
					Mu2consolPackage.Literals.POSTPROCEDURE_DIAGNOSIS_SECTION
							.getEAllOperations().get(62));
			try {
				GETMU2CONSOL_POSTPROCEDURE_DIAGNOSIS__EOCL_QRY = helper
						.createQuery(GETMU2CONSOL_POSTPROCEDURE_DIAGNOSIS__EOCL_EXP);
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(
						pe.getLocalizedMessage());
			}
		...
		OCL.Query query = EOCL_ENV
				.createQuery(GETMU2CONSOL_POSTPROCEDURE_DIAGNOSIS__EOCL_QRY);
		return (PostprocedureDiagnosis) query
				.evaluate(postprocedureDiagnosisSection);
	}
