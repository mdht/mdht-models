		...
		if (!EOCL_ENV.createQuery(VALIDATE_MEDICATION_TYPE_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					ConsolPlugin.INSTANCE.getString("MedicationTypeTemplateId"), new Object[] { medicationType }));
			}
			return false;
		}
		return true;
		...
		if (!EOCL_ENV.createQuery(VALIDATE_MEDICATION_TYPE_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(medicationType)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					ConsolPlugin.INSTANCE.getString("MedicationTypeCode"), new Object[] { medicationType }));
			}
			return false;
		}
		return true;
