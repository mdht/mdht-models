...
public class AbdomenSectionOperations extends SectionOperations {
		...
		if (!EOCL_ENV.createQuery(VALIDATE_ABDOMEN_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					ConsolPlugin.INSTANCE.getString("AbdomenSectionTemplateId"), new Object[] { abdomenSection }));
			}
			return false;
		}
		return true;
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_ABDOMEN_SECTION_PROBLEM_ENTRY__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.entry->one(entry : cda::Entry | not entry.observation.oclIsUndefined() and entry.observation.oclIsKindOf(consol::ProblemEntryReactionObservationContainer))";
	 ...
	 * @generated
	 */
	public static ProblemEntryReactionObservationContainer getProblemEntry(AbdomenSection abdomenSection) {
		if (GET_PROBLEM_ENTRY__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(
				ConsolPackage.Literals.ABDOMEN_SECTION,
				ConsolPackage.Literals.ABDOMEN_SECTION.getEAllOperations().get(58));
			try {
		...
		return (ProblemEntryReactionObservationContainer) query.evaluate(abdomenSection);
	}
