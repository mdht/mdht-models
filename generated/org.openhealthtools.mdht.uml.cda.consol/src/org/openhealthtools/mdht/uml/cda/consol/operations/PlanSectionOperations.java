...
import org.openhealthtools.mdht.uml.cda.consol.PlanSection;
 ...
 * <ul>
 *   <li>{@link org.openhealthtools.mdht.uml.cda.consol.PlanSection#validatePlanSectionTemplateId(org.eclipse.emf.common.util.DiagnosticChain, java.util.Map) <em>Validate Plan Section Template Id</em>}</li>
 *   <li>{@link org.openhealthtools.mdht.uml.cda.consol.PlanSection#validatePlanSectionCode(org.eclipse.emf.common.util.DiagnosticChain, java.util.Map) <em>Validate Plan Section Code</em>}</li>
 * </ul>
 * </p>
 *
 * @generated
...
public class PlanSectionOperations extends SectionOperations {
	...
	public static boolean validatePlanSectionTemplateId(PlanSection planSection, DiagnosticChain diagnostics,
		...
		if (VALIDATE_PLAN_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV == null) {
			...
			helper.setContext(ConsolPackage.Literals.PLAN_SECTION);
			try {
		...
		if (!EOCL_ENV.createQuery(VALIDATE_PLAN_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(planSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					ConsolValidator.PLAN_SECTION__PLAN_SECTION_TEMPLATE_ID,
					ConsolPlugin.INSTANCE.getString("PlanSectionTemplateId"), new Object[] { planSection }));
			}
			return false;
		}
		return true;
	...
	public static boolean validatePlanSectionCode(PlanSection planSection, DiagnosticChain diagnostics,
		...
		if (!EOCL_ENV.createQuery(VALIDATE_PLAN_SECTION_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(planSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
					ConsolValidator.PLAN_SECTION__PLAN_SECTION_CODE,
					ConsolPlugin.INSTANCE.getString("PlanSectionCode"), new Object[] { planSection }));
			}
			return false;
		}
		return true;
