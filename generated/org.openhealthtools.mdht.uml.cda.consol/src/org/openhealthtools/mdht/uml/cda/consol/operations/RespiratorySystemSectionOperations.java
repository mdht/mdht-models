...
"value.code = '11412-4' and value.codeSystem = '2.16.840.1.113883.6.1'))";
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_RESPIRATORY_SYSTEM_SECTION_PROBLEM_ENTRY__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.entry->one(entry : cda::Entry | not entry.observation.oclIsUndefined() and entry.observation.oclIsKindOf(consol::AllergyObservation))";
	...
	public static AllergyObservation getProblemEntry(RespiratorySystemSection respiratorySystemSection) {
		if (GET_PROBLEM_ENTRY__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(ConsolPackage.Literals.RESPIRATORY_SYSTEM_SECTION, ConsolPackage.Literals.RESPIRATORY_SYSTEM_SECTION.getEAllOperations().get(57));
			try {
		...
		return (AllergyObservation) query.evaluate(respiratorySystemSection);
	}
