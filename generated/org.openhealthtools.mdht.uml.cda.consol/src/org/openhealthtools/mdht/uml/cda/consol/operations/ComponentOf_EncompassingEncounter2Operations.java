...
public class ComponentOf_EncompassingEncounter2Operations extends EncompassingEncounterOperations {
	...
	public static boolean validateEncompassingEncounter2EffectiveTime(
		...
		if (!EOCL_ENV.createQuery(VALIDATE_ENCOMPASSING_ENCOUNTER2_EFFECTIVE_TIME__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
	...
	public static boolean validateEncompassingEncounter2Id(
		...
		if (!EOCL_ENV.createQuery(VALIDATE_ENCOMPASSING_ENCOUNTER2_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			componentOf_EncompassingEncounter2)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
	...
	public static boolean validateEncompassingEncounter2ResponsibleParty(
		...
		if (!EOCL_ENV.createQuery(VALIDATE_ENCOMPASSING_ENCOUNTER2_RESPONSIBLE_PARTY__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			componentOf_EncompassingEncounter2)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
	...
	public static boolean validateEncompassingEncounter2EncounterParticipant(
		...
		if (!EOCL_ENV.createQuery(
			VALIDATE_ENCOMPASSING_ENCOUNTER2_ENCOUNTER_PARTICIPANT__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			componentOf_EncompassingEncounter2)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.INFO,
