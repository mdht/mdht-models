...
public class HistoryOfPastIllnessSectionOperations extends SectionOperations {
	 ...
	 * @generated
	 */
	public static boolean validateHistoryOfPastIllnessSectionHasClinicalStatements(
		...
		if (!EOCL_ENV.createQuery(
			VALIDATE_HISTORY_OF_PAST_ILLNESS_SECTION_HAS_CLINICAL_STATEMENTS__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_HISTORY_OF_PAST_ILLNESS_SECTION_CONDITION__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.entry->one(entry : cda::Entry | not entry.act.oclIsUndefined() and entry.act.oclIsKindOf(consol::Condition))";
	 ...
	 * @generated
	 */
	public static boolean validateHistoryOfPastIllnessSectionCondition(
			HistoryOfPastIllnessSection historyOfPastIllnessSection, DiagnosticChain diagnostics,
			Map<Object, Object> context) {
		...
		if (!EOCL_ENV.createQuery(VALIDATE_HISTORY_OF_PAST_ILLNESS_SECTION_CONDITION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			historyOfPastIllnessSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					Diagnostic.ERROR, ConsolValidator.DIAGNOSTIC_SOURCE,
	...
	public static Condition getCondition(HistoryOfPastIllnessSection historyOfPastIllnessSection) {
		if (GET_CONDITION__EOCL_QRY == null) {
			...
			helper.setOperationContext(
				...
				ConsolPackage.Literals.HISTORY_OF_PAST_ILLNESS_SECTION.getEAllOperations().get(59));
			try {
