...
public class EarsSectionOperations extends SectionOperations {
		...
		if (!EOCL_ENV.createQuery(VALIDATE_EARS_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(earsSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					ConsolPlugin.INSTANCE.getString("EarsSectionTemplateId"), new Object[] { earsSection }));
			}
			return false;
		}
		return true;
		...
		if (!EOCL_ENV.createQuery(VALIDATE_EARS_SECTION_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(earsSection)) {
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					ConsolPlugin.INSTANCE.getString("EarsSectionCode"), new Object[] { earsSection }));
			}
			return false;
		}
		return true;
	 ...
	 * @ordered
	 */
	protected static final String VALIDATE_EARS_SECTION_PROBLEM_ENTRY__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP = "self.entry->one(entry : cda::Entry | not entry.observation.oclIsUndefined() and entry.observation.oclIsKindOf(consol::AllergyIntolerance))";
		...
		if (!EOCL_ENV.createQuery(VALIDATE_EARS_SECTION_PROBLEM_ENTRY__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(
			...
			if (diagnostics != null) {
				diagnostics.add(new BasicDiagnostic(
					...
					ConsolPlugin.INSTANCE.getString("EarsSectionProblemEntry"), new Object[] { earsSection }));
			}
			return false;
		}
		return true;
	 ...
	 * @generated
	 */
	public static AllergyIntolerance getProblemEntry(EarsSection earsSection) {
		if (GET_PROBLEM_ENTRY__EOCL_QRY == null) {
			OCL.Helper helper = EOCL_ENV.createOCLHelper();
			helper.setOperationContext(
				ConsolPackage.Literals.EARS_SECTION, ConsolPackage.Literals.EARS_SECTION.getEAllOperations().get(58));
			try {
		...
		return (AllergyIntolerance) query.evaluate(earsSection);
	}
