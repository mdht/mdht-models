...
public class CommentOperations extends ClinicalStatementOperations {
			...
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						...
						CCDPlugin.INSTANCE.getString("CommentCommentTemplateId"), new Object[] { comment }));
			}
	...
	public static boolean validateCommentClassCode(Comment comment, DiagnosticChain diagnostics,
		...
		if (VALIDATE_COMMENT_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(CCDPackage.Literals.COMMENT);
			try {
				VALIDATE_COMMENT_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_COMMENT_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}

		if (!EOCL_ENV.get().createQuery(VALIDATE_COMMENT_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
			comment)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, CCDValidator.DIAGNOSTIC_SOURCE, CCDValidator.COMMENT__COMMENT_CLASS_CODE,
						CCDPlugin.INSTANCE.getString("CommentCommentClassCode"), new Object[] { comment }));
			}
	...
	public static boolean validateCommentMoodCode(Comment comment, DiagnosticChain diagnostics,
		...
		if (VALIDATE_COMMENT_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(CCDPackage.Literals.COMMENT);
			try {
				VALIDATE_COMMENT_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_COMMENT_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}

		if (!EOCL_ENV.get().createQuery(VALIDATE_COMMENT_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(
			comment)) {
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						Diagnostic.ERROR, CCDValidator.DIAGNOSTIC_SOURCE, CCDValidator.COMMENT__COMMENT_MOOD_CODE,
						CCDPlugin.INSTANCE.getString("CommentCommentMoodCode"), new Object[] { comment }));
			}
	 ...
	 * @generated
	 */
	public static boolean validateCommentCode(Comment comment, DiagnosticChain diagnostics,
			Map<Object, Object> context) {
		...
		if (VALIDATE_COMMENT_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get() == null) {
			...
			helper.setContext(CCDPackage.Literals.COMMENT);
			try {
				VALIDATE_COMMENT_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.set(
					helper.createInvariant(VALIDATE_COMMENT_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_EXP));
			} catch (ParserException pe) {
				throw new UnsupportedOperationException(pe.getLocalizedMessage());
			}
		}
			...
			if (diagnostics != null) {
				diagnostics.add(
					new BasicDiagnostic(
						...
						CCDPlugin.INSTANCE.getString("CommentCommentCode"), new Object[] { comment }));
			}
