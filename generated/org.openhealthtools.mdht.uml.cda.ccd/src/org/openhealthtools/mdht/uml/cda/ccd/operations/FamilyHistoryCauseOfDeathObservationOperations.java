...
public class FamilyHistoryCauseOfDeathObservationOperations extends FamilyHistoryObservationOperations {
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_FAMILY_HISTORY_CAUSE_OF_DEATH_OBSERVATION_CAUSE_OF_DEATH_OBSERVATION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(familyHistoryCauseOfDeathObservation))
    {
      if (diagnostics != null)
      {
        diagnostics.add
   ...
   */
    ...
    {
      OCL.Helper helper = EOCL_ENV.createOCLHelper();
      helper.setOperationContext(CCDPackage.Literals.FAMILY_HISTORY_CAUSE_OF_DEATH_OBSERVATION, CCDPackage.Literals.FAMILY_HISTORY_CAUSE_OF_DEATH_OBSERVATION.getEAllOperations().get(68));
      try
      {
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_FAMILY_HISTORY_OBSERVATION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(familyHistoryCauseOfDeathObservation))
    {
      if (diagnostics != null)
      {
        diagnostics.add
          (new BasicDiagnostic
            (Diagnostic.ERROR,
             ...
             org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString("_UI_GenericInvariant_diagnostic", new Object[] { "FamilyHistoryObservationTemplateId", org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(familyHistoryCauseOfDeathObservation, context) }),
             new Object [] { familyHistoryCauseOfDeathObservation }));
      }
