...
public class AdvanceDirectivesSectionOperations extends CodedAdvanceDirectivesSectionOperations {
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_HITSP_ADVANCE_DIRECTIVES_SECTION_ADVANCE_DIRECTIVE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(advanceDirectivesSection))
    {
      if (diagnostics != null)
      {
        diagnostics.add
   ...
   */
    ...
    {
      OCL.Helper helper = EOCL_ENV.createOCLHelper();
      helper.setOperationContext(HITSPPackage.Literals.ADVANCE_DIRECTIVES_SECTION, HITSPPackage.Literals.ADVANCE_DIRECTIVES_SECTION.getEAllOperations().get(67));
      try
      {
