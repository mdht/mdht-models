...
public class ReasonForReferralSectionOperations extends CodedReasonForReferralSectionOperations {
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_HITSP_REASON_FOR_REFERRAL_SECTION_CONDITION__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(reasonForReferralSection))
    {
      if (diagnostics != null)
      {
        diagnostics.add
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_HITSP_REASON_FOR_REFERRAL_SECTION_RESULT__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(reasonForReferralSection))
    {
      if (diagnostics != null)
      {
        diagnostics.add
   ...
   */
    ...
    {
      OCL.Helper helper = EOCL_ENV.createOCLHelper();
      helper.setOperationContext(HITSPPackage.Literals.REASON_FOR_REFERRAL_SECTION, HITSPPackage.Literals.REASON_FOR_REFERRAL_SECTION.getEAllOperations().get(66));
      try
      {
   ...
   */
    ...
    {
      OCL.Helper helper = EOCL_ENV.createOCLHelper();
      helper.setOperationContext(HITSPPackage.Literals.REASON_FOR_REFERRAL_SECTION, HITSPPackage.Literals.REASON_FOR_REFERRAL_SECTION.getEAllOperations().get(67));
      try
      {
   ...
   */
    ...
    if (!EOCL_ENV.createQuery(VALIDATE_CODED_REASON_FOR_REFERRAL_SECTION_TEMPLATE_ID__DIAGNOSTIC_CHAIN_MAP__EOCL_INV).check(reasonForReferralSection))
    {
      if (diagnostics != null)
      {
        diagnostics.add
          (new BasicDiagnostic
            (Diagnostic.ERROR,
             ...
             org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString("_UI_GenericInvariant_diagnostic", new Object[] { "CodedReasonForReferralSectionTemplateId", org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(reasonForReferralSection, context) }),
             new Object [] { reasonForReferralSection }));
      }
