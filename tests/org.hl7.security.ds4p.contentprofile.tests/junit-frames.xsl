...
<xsl:template name="display-failxml">
	...
	<xsl:choose>
  		...
  		<xsl:otherwise>
			<xsl:copy-of select="document( concat('target/surefire-reports/','allvalidationresults.xml') )/testresults/testresult[contains(@name, $value)]/failsnippet" />
  		</xsl:otherwise>
	</xsl:choose>
    </p>  	
</xsl:template>
...
<xsl:template name="display-passxml">
   ...
   <xsl:value-of select="$value"/>
	<xsl:choose>
  		...
  		foo2
  		<xsl:text>
  		document( concat(substring ('target/surefire-reports/', 3),'allvalidationresults.xml') )/testresults/testresult[@name=$value]/passsnippet
  		</xsl:text>
